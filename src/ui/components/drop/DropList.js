      ...
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto">
          <h1 className="text-2xl font-bold text-gray-900">
            {isCurrentUser ? `Starlinks Verificadas (${drops.length})` : `DROPs (${drops.length})`}
          </h1>
        </div>
      ...
      {drops.length > 0 ?
        ...
        <div className="flex mb-10 justify-center">
          <label className="leading-[200px] font-flow font-medium text-base text-gray-500">
            {isCurrentUser ? "Ainda não tem nenhuma starlink verificada" : "This account haven't created DROP yet"}
          </label>
        </div>}
    </div>
  )
